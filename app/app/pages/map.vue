...
const { data: city, refresh, status } = await useLiveCityAir({ city: 'kyiv' });
...
function openFullMap() {
    window.open(city.value?.link_maps_aqi ?? MAP_EMBED_URL, '_blank', 'noopener,noreferrer');
}
</script>
...
<template>
    <div class="space-y-6">
        <section class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <h2 class="text-2xl font-semibold">Kyiv Air Quality Map</h2>
        ...
        <section v-if="city" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div class="card">
            ...
            <div class="card">
                <p class="text-xs uppercase text-slate-400">Wind</p>
                <p class="kpi-value">{{ city.meteo.wind_power.value }} m/s</p>
                <p class="text-xs text-slate-500">
                    {{ city.meteo.wind_direction.value }}
                </p>
            </div>
            <div class="card">
                ...
                <p class="mt-1 text-xs text-slate-500">NowCast (US EPA)</p>
            </div>
        </section>
        ...
        <div v-else class="card flex h-24 items-center justify-center text-sm text-red-400">
        ...
        <div class="card overflow-hidden p-0">
            ...
            >
                <p class="text-sm text-slate-400">City-wide average AQI PM2.5 (NowCast, US EPA)</p>
                <button type="button" class="text-sm text-emerald-400 hover:underline" @click="openFullMap">
                    Open full map ↗
                </button>
            </div>
            <iframe
                :src="MAP_EMBED_URL"
                title="SaveEcoBot city air quality map"
